    ...
    , enabled_(false)
{
    ...
    for (ParticipantId id: ids)
    {
        ...
        // if (nullptr != service_registry)
        {
            ...
            {
                // tsnh or exception
                logError(DDSROUTER_BRIDGE, "Huge error happenned while creating service Bridge for topic" << topic);
            }
            ...
            // if (id == service_registry->server_participant_id() && participant->is_rtps_kind())
            {
                service_registry->set_related_sample_identity(std::static_pointer_cast<rtps::Reader>(readers_[id])->guid());
            }
        }
        else
        {
