        ...
        const Endpoint& endpoint) noexcept
{
    ...
    {
        if ((endpoint.is_reader() && ServiceRegistry::is_request_topic(topic)) ||
                (endpoint.is_writer() && ServiceRegistry::is_reply_topic(topic)))
        {
        ...
        const types::ParticipantId& server_participant_id) noexcept
{
    ...
    try
    {
        bridges_[reply_topic] = std::make_unique<Bridge>(reply_topic, participants_database_, payload_pool_,
                        thread_pool_, false, service_registry);
    }
    catch (const utils::InitializationException& e)
    {
        logError(DDSROUTER,
    ...
    try
    {
        bridges_[request_topic] = std::make_unique<Bridge>(request_topic, participants_database_, payload_pool_,
                        thread_pool_, false, service_registry);
    }
    catch (const utils::InitializationException& e)
    {
