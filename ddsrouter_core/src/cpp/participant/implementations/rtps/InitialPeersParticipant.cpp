        ...
        const configuration::InitialPeersParticipantConfiguration* configuration)
{
    ...
    for (const types::Address& address : configuration->listening_addresses)
    {
        ...
        eprosima::fastrtps::rtps::IPLocator::setPhysicalPort(locator, address.external_port_);

        // In TCP case, set Physical port
        if (address.is_tcp())
        {
            eprosima::fastrtps::rtps::IPLocator::setLogicalPort(locator, address.external_port_);
        }
